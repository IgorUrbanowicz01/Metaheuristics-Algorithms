...
def getNeighbours(permutation, taboo_swaps, type):
    neighbours = []
    for i in range(len(permutation) - 1):
        for j in range(i + 1, len(permutation)):
            if [i, j] not in taboo_swaps:
                if type == "invert":
                    neighbours.append(invert(permutation, i, j))
                elif type == "swap":
                    neighbours.append(swap(permutation, i, j))
                elif type == "insert":
                    neighbours.append(insert(permutation, i, j))
                elif type == "reverse insert":
                    neighbours.append(reverse_insert(permutation, i, j))
                else:
                    print("Źle podany typ")
    return neighbours


def getBestNeighbour(permutation, distance_matrix, taboo_list, type):
    neighbours = getNeighbours(permutation, taboo_list, type)
    if len(neighbours) == 0:
    ...
    for n in neighbours:
        if calculateDistance(n, distance_matrix) < bestDistance:
            ...
            bestDistance = calculateDistance(bestNeighbour, distance_matrix)
    # print(bestNeighbour)
    # print(calculateDistance(bestNeighbour, distance_matrix))
    return bestNeighbour


def tabooSearch(distance_matrix, taboo_length, before_jump_count, search_depth, type):
    resoult = 0
    ...
    for i in range(search_depth):
        # print(got_worse_solution_count)
        new_permutation = getBestNeighbour(permutation, distance_matrix, taboo_list, type).copy()
        ...
        if got_worse_solution_count >= before_jump_count:     # Should we return to last best solution
            ...
            where_to_next = getPermutationDifference(
                getBestNeighbour(new_permutation, distance_matrix, taboo_list, type), new_permutation)
