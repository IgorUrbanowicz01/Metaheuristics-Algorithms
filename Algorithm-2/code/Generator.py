...
def tabu_list_size_generator(city):
    resoults_tabuint = []
    tabuint = 0
    resoults_tabulog = []
    tabulog = 0
    ...
    while city_count < city:
        for i in range(0, 10):
            print(i)
            distanceMatrix = randomMatrixSym(city_count, 10, 200, random.randint(1, 1000))
            tabuint += tabooSearch(distanceMatrix, 5, 10, 200, "swap")
            tabulog += tabooSearch(distanceMatrix, int(np.log(city_count)), 10, 200, "swap")
            tabun += tabooSearch(distanceMatrix, city_count, 10, 200, "swap")
            tabun2 += tabooSearch(distanceMatrix, city_count*city_count, 10, 200, "swap")
        minimumprd = min(tabuint, tabulog, tabun, tabun2)
        resoults_tabuint.append(tabuint/minimumprd-1)
        resoults_tabulog.append(tabulog/minimumprd-1)
        resoults_tabun.append(tabun/minimumprd-1)
        resoults_tabun2.append(tabun2/minimumprd-1)
        print(ichart)
        print(resoults_tabuint)
        print(resoults_tabulog)
        print(resoults_tabun)
        print(resoults_tabun2)
        tabuint = 0
        tabulog = 0
        tabun = 0
        tabun2 = 0
        city_count += 10
        k += 1
    create_all_chart_tabu(ichart, resoults_tabuint, resoults_tabulog, resoults_tabun, resoults_tabun2, "tabu = 5","tabu = log(city)", "tabu = city", "tabu = city^2","numer of cities", "best rout","How the size of tabu effects best rout")
    show_chart()